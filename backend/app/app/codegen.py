...
env = Environment(
    loader=PackageLoader("app", package_path="codegen"), autoescape=select_autoescape()
)
...
def install_files(model_name):
    ...
    os.rename(
        ...
        f"api/api_v1/endpoints/{pn}.py",
    )
    os.rename(
        f"codegen/generated/crud_{camel_to_snake(model_name)}.py",
        f"crud/crud_{camel_to_snake(model_name)}.py",
    )
    os.rename(
        f"codegen/generated/schemas_{camel_to_snake(model_name)}.py",
        f"schemas/{camel_to_snake(model_name)}.py",
    )
    os.rename(
        f"codegen/generated/test_{camel_to_snake(model_name)}.py",
        f"tests/api/api_v1/test_{camel_to_snake(model_name)}.py",
    )
...
def main(model_name: str):
    ...
    save_to_file(
        crud_template.render(
            entity=model_name, entity_lower=camel_to_snake(model_name), pn=pn
        ),
        f"codegen/generated/crud_{camel_to_snake(model_name)}.py",
    )
    if "owner" in related_fields:
