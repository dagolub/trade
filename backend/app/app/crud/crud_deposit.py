...
class CRUDDeposit(CRUDBase[Deposit, DepositCreate, DepositUpdate]):
    ...
    async def create(self, db: Session, obj_in: dict, owner=None) -> Optional[ModelType]:
        exchanger = Exchanger()
        ...
        wallet = okx.get_address(sub_account, obj_in.currency)
        ...
        currency = obj_in.currency.lower()
        passphrase = generate_random_string_passphrase(16)
        sub = okx.create_sub_account_api_key(sub_account, sub_account + "Label", passphrase)
        obj_in = {
            "owner_id": owner["id"],
            "wallet": wallet,
            "type": deposit_type,
            "sum": okx.frac_to_int(deposit_sum, currency),
        ...
        await crud.wallet.create(db=db, obj_in=WalletCreate(owner_id=owner["id"],
