...
API_KEY = settings.MEXC_KEY
SECRET_KEY = settings.MEXC_SECRET
...
def _sign_v2(sign_params=None):
    ...
    params = hmac.new(SECRET_KEY.encode(), params.encode(), hashlib.sha256).hexdigest()
    return params
...
def delete_cancel(order_ids=None, client_order_ids=None):
    ...
    data = "&".join("{}={}".format(i, data_orignal[i]) for i in sorted(data_orignal))
    params = _sign_v2(sign_params=data)
    headers = {
        "ApiKey": API_KEY,
        "Request-Time": str(_get_server_time()),
...
def get_deposit_list(
    currency=None, startTime=None, endTime=None, pageSize=None, pagenum=None, state=None
):
