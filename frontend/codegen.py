...
def install_files(model_name, entity_lower, schema_fields):
    ...
    if entity_lower not in sidebar_file:
        replace_to = "<li className={li_class}>\n" \
                    ...
                    f"            {model_name}s\n" \
                     ...
                     "    </a>\n" \
                     "</li>"
        sidebar_file = re.sub("Users</span></a></li>", f"\g<0>\n{replace_to}", sidebar_file)  # noqa
    ...
    if entity_lower not in app_file:
        replace_to = f"import {model_name}s from './pages/{entity_lower}/list';\n" \
        ...
        replace_to = f"        <Route path=\"/" + entity_lower + "/list\" element={<" + model_name + "s />} />\n" \
                     f"        <Route path=\"/" + entity_lower + "/new\" element={<" + model_name + "sNew />} />\n" \
                     f"        <Route path=\"/" + entity_lower + "/edit/:id\" element={<" + model_name + "sEdit />} />"
    ...
    if model_name not in api_file:
        replace_to = f"const create{model_name} = ({fields_names})" + " => {\n" \
    ...
    os.rename(f"codegen/generated/components_form_{entity_lower}.jsx", f"src/components/{entity_lower}/form.jsx")
    os.rename(f"codegen/generated/components_table_{entity_lower}.jsx",
              f"src/components/{entity_lower}/{model_name}sTable.jsx")
    os.rename(f"codegen/generated/components_table_item_{entity_lower}.jsx",
    ...
    os.rename(f"codegen/generated/pages_edit_{entity_lower}.jsx", f"src/pages/{entity_lower}/edit.jsx")
    os.rename(f"codegen/generated/pages_list_{entity_lower}.jsx",
              f"src/pages/{entity_lower}/list.jsx")
    os.rename(f"codegen/generated/pages_new_{entity_lower}.jsx",
...
def main(model_name: str):
    ...
    save_to_file(components_form.render(entity=model_name,
    ...
    save_to_file(components_entity_table.render(entity=model_name,
                                                entity_lower=entity_lower,
                                                schema_fields=schema_fields,
                                                schema_fields_capitalize=schema_fields_capitalize
                                                ),
    ...
    components_entity_table_item = env.get_template("components/EntityTableItem.jsx.tpl")
    save_to_file(components_entity_table_item.render(entity=model_name,
                                                     entity_lower=entity_lower,
                                                     schema_fields=schema_fields,
                                                     schema_fields_capitalize=schema_fields_capitalize
                                                     ),
    ...
    save_to_file(pages_edit.render(entity=model_name,
                                   entity_lower=entity_lower,
                                   schema_fields=schema_fields,
                                   schema_fields_capitalize=schema_fields_capitalize
                                   ),
    ...
    save_to_file(pages_list.render(entity=model_name,
                                   entity_lower=entity_lower,
                                   schema_fields=schema_fields,
                                   schema_fields_capitalize=schema_fields_capitalize
                                   ),
    ...
    save_to_file(pages_new.render(entity=model_name,
                                  entity_lower=entity_lower,
                                  schema_fields=schema_fields,
                                  schema_fields_capitalize=schema_fields_capitalize
                                  ),
