...
function {{ entity }}TableItem(props) {
    ...
    return (
        <tr id={"tr" + props.id}>
            ...
            {% for field in schema_fields %}<td className={"px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap td" + props.id}>
                <div className="font-medium text-slate-800 dark:text-slate-100">{props.{{ field }}}</div>
            </td>
            {% endfor %}
            <td className={"px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px td" + props.id}>
                <div className="space-x-1">
                    <a href={"/{{ entity_lower }}/edit/" + props.id}>
                        <button
...
export default {{ entity }}TableItem;
