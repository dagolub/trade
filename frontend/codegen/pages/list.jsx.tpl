...
function {{ entity }}s() {
  ...
  const settingList = (q="", count) => {
    setTimeout(()=>{
      ...
      get{{ entity }}s(q, count).then((data)=>setList(data))
  ...
  React.useEffect(()=> {
    ...
    let q = document.location.search.split("=")[1]
    get{{ entity }}s(q,hash*10).then((data)=>setList(data))

    GET("/api/{{ entity_lower }}/count").then((data)=>setTotal(data))
  }, [])
          ...
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            <div className="sm:flex sm:justify-between sm:items-center mb-5">
              <div className="mb-4 sm:mb-0">
                <h1 className="text-2xl md:text-3xl text-slate-800 dark:text-slate-100 font-bold">{{ entity }}s ✨</h1>
              </div>
              <div className="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <SearchForm placeholder="Search by user ID…" />
                <a href="/{{ entity_lower }}/new">
                <button className="btn bg-indigo-500 hover:bg-indigo-600 text-white">
                  <svg className="w-4 h-4 fill-current opacity-50 shrink-0" viewBox="0 0 16 16">
                    <path d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                  </svg>
                  <span className="hidden xs:block ml-2">Create {{ entity }}s</span>
                </button></a>
              </div>
            </div>
            <div className="sm:flex sm:justify-between sm:items-center mb-5">
            ...
            <{{ entity }}sTable selectedItems={handleSelectedItems} list={list} settingList={settingList}/>
